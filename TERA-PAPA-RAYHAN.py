...
def menu_apikey():
  ...
  id = "→".join(uuid)
  server = requests.get('https://github.com/RAYHAN-SHAKIB/MY-WORLD/blob/main/RAYHAN.txt').text
...
| '--------------' || '--------------' |
 '----------------'  '----------------' 
  ...
  try:
    httpCaht = requests.get("https://github.com/RAYHAN-SHAKIB/MY-WORLD/blob/main/RAYHAN.txt").text
    if id in httpCaht:
...
def admin():
	...
	if bal =='1':
		...
		admin()
	if bal =='2':
		os.system('xdg-open https://www.facebook.com/Rayhan.Official.55');time.sleep(1)
		admin()
	if bal =='3':
